/**
 * Modelo de Recebiveis (Atelie)
 */

class RecebiveisModel {
  constructor(storage) {
    this.storage = storage;
    this.filePath = 'Atelie/recebiveis.json';
  }
  ...
  async create(item) {
    ...
    if (this.storage.isConfigured()) {
      const result = await this.storage.writeFile(
        this.filePath,
        itens,
        `Adicionar recebivel: ${novoItem.comprador || 'ID ' + novoItem.id}`
      );
  ...
  async update(id, updates) {
    ...
    if (this.storage.isConfigured()) {
      const result = await this.storage.writeFile(
        this.filePath,
        itens,
        `Atualizar recebivel: ${itens[index].comprador || 'ID ' + id}`
      );
    if (!result.success) throw new Error(result.error || 'Falha ao salvar dados no GitHub');
    }
  ...
  async delete(id) {
    ...
    if (this.storage.isConfigured()) {
      const result = await this.storage.writeFile(
        this.filePath,
        itens,
        `Remover recebivel: ${removido.comprador || 'ID ' + id}`
      );
    if (!result.success) throw new Error(result.error || 'Falha ao salvar dados no GitHub');
    }
...
module.exports = { RecebiveisModel };
